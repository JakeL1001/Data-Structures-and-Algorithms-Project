...
public class MyProject implements Project {
    ...
    public boolean bfs(int[][] adjlist, int[][] rspeeds, int src, int dst, int[] parent) { //calls bfs from src to dst whilst there is still bandwidth left
        ...
        while (!queue.isEmpty()) { //Searches the graph to find augmenting paths, which are then measured for their bandwitch in the method above
            int currentnode = queue.pop();
            int counter = 0;
            for (int neighbour : adjlist[currentnode]) {
                if (!visited[neighbour] && rspeeds[currentnode][counter] > 0) { //if the node isnt visited, and it still has bandwidth
                    if (neighbour == dst) {
                        parent[neighbour] = currentnode; //If the destination is reached, the parent of the destination is set to the current node
                        return true; //true is returned as there is still a path with bandwidth left to add
                    }
                    queue.add(neighbour);
                    parent[neighbour] = currentnode; //adds the node to the parent list if not the destination
                    visited[neighbour] = true;
                }
                counter++; //loops through the neighbours of each node
            }
        }
        return false; //returns false if no paths are left with sufficient bandwidth
    }
